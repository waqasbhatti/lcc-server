...
API key associated with an LCC-Server account, and this API key is provided in
`apikey` parameter, any objects, datasets, or collections marked as **private**
or **unlisted** will become visible to the LCC-Server account that created them.
...
accessed using an HTTP request to:

```
GET {{ server_url }}/api/collections[?apikey=<optional API key>]
```
...
LCC server. The service can be accessed via an HTTP equest to:

```
GET {{ server_url }}/api/datasets[?apikey=<optional API key>]
```
...
```
GET {{ server_url }}/set/[setid]?json=1[&apikey=<optional API key>]
```
...
```
GET {{ server_url }}/api/object[?apikey=<optional API key>]
```
...
Use the value of the `apikey` item in the HTTP headers of any subsequent
